...
**Contexte :** Campagnes de marketing direct bancaire portugais

- **Train :** 45 211 observations, 17 variables
- **Test :** 4 521 observations, 17 variables
- **Objectif :** Prédire la souscription à un dépôt à terme (`y`)
:::
...
- **age** : Âge (numérique)
- **job** : Emploi (12 catégories : admin, management, technician, etc.)
- **marital** : Statut marital (married, divorced, single)
- **education** : Niveau d'éducation (primary, secondary, tertiary, unknown)
- **default** : Défaut de crédit (yes/no)
- **balance** : Équilibre annuel moyen en € (numérique)
:::
...
- **loan** : Emprunt personnel (yes/no)
- **contact** : Mode de contact (cellular, telephone, unknown)
- **day** : Jour du dernier contact (numérique)
- **month** : Mois du dernier contact (jan-dec)
- **duration** : Durée du dernier appel en sec. (numérique)
:::

### Variables campagne

::: {.smaller style="font-size:0.75em;"}
- **campaign** : Nb d'appels cette campagne (numérique)
- **pdays** : Jours depuis dernier contact campagne précédente (-1 = jamais contacté)
- **previous** : Nb d'appels campagne précédente (numérique)
- **poutcome** : Résultat campagne précédente (success, failure, other, unknown)

**Variable cible :**
- **y** : Souscription dépôt à terme (**yes/no**)
:::
:::
...
```r
# Conversion en facteurs avec niveaux ordonnés
...
```r
# Recodage des variables (suppression "unknown" et conversion binaire)
data$job <- fct_recode(data$job, NULL = "unknown")
data$education <- fct_recode(data$education, NULL = "unknown")
data$default <- recode_factor(data$default, "yes" = "1", "no" = "0")
data$housing <- recode_factor(data$housing, "yes" = "1", "no" = "0")
