...
```{r, include=FALSE}
ranger_tune_res %>% show_best("accuracy")
```
...
::: {.column .smaller width="50%"}
```{r}
final_rf_model_test %>% roc_curve(truth = y, estimate = .pred_no) %>% autoplot()
```
:::
:::::
...
```{r, include=FALSE}
boostB_tune_res %>% show_best("accuracy")
...
```{r}
roc_auc(boost_fit_on_test, truth = y, estimate = .pred_no)
```
:::
...
```{r}
boost_fit_on_test %>% roc_curve(truth = y, estimate = .pred_no) %>% autoplot()
```
